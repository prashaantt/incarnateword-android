 ...
 */
public class VolListSriAuorActivity extends BaseActivity implements ObservableScrollViewCallbacks {
...
//observable List
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // setContentView(R.layout.sri_aurbvol_list_fragment);
        getLayoutInflater().inflate(R.layout.sri_aurbvol_list_fragment, frameLayout);
        Bundle gt = getIntent().getExtras();
        SMsg = gt.getString("STRING");

//action bar heading
        if ((SMsg != null && !SMsg.equals("")) && SMsg.equals(Constant.COMP_WORK)) {
            setActionBarTitle(VolListSriAuorActivity.this, getString(R.string.complete_work), getSupportActionBar());
        } else {
            setActionBarTitle(VolListSriAuorActivity.this, getString(R.string.birth_cent_lib), getSupportActionBar());
        }

        pb = (ProgressBar) findViewById(R.id.progressBar1);
        VolHeader = (TextView) findViewById(R.id.txtvolheader);

        if (Typefaces.get(VolListSriAuorActivity.this, "CharlotteSans_nn") != null) {
            VolHeader.setTypeface(Typefaces.get(VolListSriAuorActivity.this, "CharlotteSans_nn"));
        }
        ...
        View paddingView = new View(VolListSriAuorActivity.this);
        ...
        if (Utils.haveNetworkConnection(VolListSriAuorActivity.this)) {
            if (SMsg != null && !SMsg.equals("")) {
                makeJsonObjectRequest();
            }
        } else {
            Toast.makeText(VolListSriAuorActivity.this, getResources().getString(R.string.InternetConnection), Toast.LENGTH_SHORT).show();
        }
    ...
    public void onDrawerItemSelected(View view, int position) {
        if (SMsg.equals(Constant.BIRTH_CEN_LIB) && position == 2) {
            return;
        }
        if (SMsg.equals(Constant.COMP_WORK) && position == 3) {
            return;
        }

        ChangeActivity(VolListSriAuorActivity.this, position);
    }
            ...
            public void run() {
                try {
                    ...
                    if (list != null && !list.isEmpty()) {
                        try {
                            VolHeader.setText("");
                            adt = new BirthCenVolAdapter(VolListSriAuorActivity.this, list, false);
                            mListView.setAdapter(adt);

                            if (Heading != null && !Heading.equals("")) {
                                setActionBarTitle(VolListSriAuorActivity.this, Heading, getSupportActionBar());
                            }
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
    ...
    public void onBackPressed() {
        super.onBackPressed();
        VolListSriAuorActivity.this.finish();
    }
